...
async function loadBannerData(): Promise<void> {
    try {
      const response = await fetch("http://127.0.0.1:8000/api/v1/banner/");
      if (!response.ok) throw new Error("No se pudo obtener la información del banner");

      const data = await response.json();
      if (data.length === 0) throw new Error("No se encontraron datos en la respuesta");

      const { svg, img, img_alt } = data[0];
      updateBannerContent(svg, img, img_alt);
    } catch (error) {
      console.error("Error al obtener los datos del banner, utilizando imágenes de respaldo.", error);
...
function updateBannerContent(svgUrl: string, imgUrl: string, imgAlt: string) {
    ...
    image.alt = imgAlt;
    image.loading = "eager";
    image.className = "w-full absolute top-1/2 transform -translate-y-1/2 animate-fade animate-duration-2500 animate-delay-2000 animate-ease-out animate-reverse";
...
// Wrap all the initialization code in an async IIFE
(async function initializeApp() {
    ...
    if (bgBanner && camion && titulo) {
      ...
      tl.add('#titulo', {
        translateX: ['-100%', '0%'],
        duration: 4000
      }, 2000);  // starts at the same time as the previous animation
      ...
      tl.add('#camion', {
        ...
        onBegin: function () {
          ...
          const contorno = document.getElementById("contorno") as SVGPathElement | null;
          const tanqueCabina = document.getElementById("tanqueCabina") as SVGPathElement | null;
          const llantas = document.getElementById("llantas") as SVGPathElement | null;
          const tanqueGas = document.getElementById("tanqueGas") as SVGPathElement | null;
      ...
      // Animación del botón
      tl.add('.btn-secondary', {
        opacity: [0, 1],
        duration: 2000,
        easing: 'linear'
      }, '<-=2000');  // starts 1000ms after the previous animation
